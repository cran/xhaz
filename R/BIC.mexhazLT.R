#' @title Bayesian Information Criterion for excess hazard model from mexhazLT function
#'
...
BIC.mexhazLT <- function(object, ...) {

  dots.object <- list(...)
  if (length(dots.object) == 0) {
    if (inherits(object, "mexhazLT")) {
      df <- object$n.par
      val <- (log((object$n.obs)) * object$n.par - 2 * (object$loglik))
    } else{
    ...
    aic_bis <- function(i) {
      if (inherits(object[[i]], "mexhazLT")) {
        ...
        val <-
          (log((object[[i]]$n.obs)) * object[[i]]$n.par - 2 * (object[[i]]$loglik))
